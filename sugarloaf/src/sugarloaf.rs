...
use cosmic_text::{Edit, Weight, Style};
        ...
        for i in 0..size {
            ...
            if let Some(style) = &stack[i].style {
                if style.is_bold_italic {
                    self.spans.push((sugar_str, attr.weight(cosmic_text::Weight::BOLD).style(Style::Italic)));
                } else if style.is_bold {
                    self.spans.push((sugar_str, attr.weight(cosmic_text::Weight::BOLD)));
